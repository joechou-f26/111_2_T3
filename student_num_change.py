...
df1['total_diff']=df1['total'].diff()           # difference between two rows
df1['total_diff'] = df1['total_diff'].fillna(0) # fill NAN with 0
...
ax1.set_ylabel('人', color='r',fontproperties=myFont) 
ax1.legend(loc="upper left",prop=myFont) 
if b_pct_change:
    # second axes with unit 'change %'
    ax2 = ax1.twinx()  # Create a twin Axes sharing the xaxis
    ax2.plot(xpos, df1.total_diff, 'or-', label='增減人數') 
    ax2.set_ylabel('人數', color='r',fontproperties=myFont)  
...
for i, v in enumerate(y_val):
    ax1.text(i,v, '{:,.0f}'.format(v), color='blue', horizontalalignment='center')
y_val=df1['total_diff'].values.tolist()
if b_pct_change:
    for i, v in enumerate(y_val):
        ax2.text(i,v,  '{:,.0f}'.format(v), color='red', horizontalalignment='right')
plt.show()
